  ...
  schema: { type: 'object', additionalProperties: false, required: ['briefs'],
    properties: { briefs: { type: 'array', items: { type: 'object', additionalProperties: false,
      ...
      properties: {
        ...
        article_topic: { type: 'string', description: 'SEO listicle title with the keyword in the FIRST half + a SPECIFIC promise and a number, e.g. "12 Small Bedroom Decor Ideas That Make a Rental Feel Cozy". NEVER vague like "Best decor tips".' },
        pin_titles: { type: 'array', items: { type: 'string' }, description: '3 pin titles (<=95 chars). Each: keyword in the first half + a SPECIFIC, save-worthy promise (a number, a concrete outcome, or a "for [who/where]"). People save pins that promise something specific — never vague "best tips" titles.' },
        pin_descriptions: { type: 'array', items: { type: 'string' }, description: '2 pin descriptions: keyword in the first sentence, ONE specific benefit, then a clear CTA. Natural prose, not keyword-stuffed.' },
